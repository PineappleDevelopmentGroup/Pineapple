...
class AttributeModifierAdapter implements SerializedAdapter<AttributeModifier> {
    ...
    @Override
    public SerializedElement serialize(final AttributeModifier attributeModifier, final SerializedSerializeContext context) throws SerializedAdaptationException {
    ...
    @Override
    public AttributeModifier deserialize(final SerializedElement element, final SerializedDeserializeContext context) throws SerializedAdaptationException {
        if (!element.isObject()) {
        ...
        final AttributeModifier.Operation operation = AttributeModifier.Operation.valueOf(
            object.getPrimitive(OPERATION).orThrow().getAsString());
        final EquipmentSlotGroup slotGroup = EquipmentSlotGroup.getByName(
            object.getPrimitive(EQUIPMENT_SLOT_GROUP).orThrow().getAsString());
