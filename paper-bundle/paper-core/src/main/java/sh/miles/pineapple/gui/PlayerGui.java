...
public abstract class PlayerGui<T extends InventoryView> {
    ...
    protected PlayerGui(@NotNull final Function<Player, T> view, @NotNull final Player viewer) {
        Preconditions.checkArgument(view != null, "The given scene must not be null");
        Preconditions.checkState(viewer != null, "The given viewer must not be null");
        this.view = view.apply(viewer);
        Preconditions.checkState(this.view != null, "The given scene function must return a non null value");
        this.viewer = viewer;
        this.topInventory = this.view.getTopInventory();
